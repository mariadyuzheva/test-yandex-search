...
def test_text_search(browser):
    """Проверяет наличие поля поиска, таблицы с подсказками и результатов поиска на yandex.ru"""
    ...
    assert main_page.is_search_suggest_present(), "Отсутствует таблица с подсказками (suggest)"

    # 5. При нажатии Enter появляется таблица результатов поиска
    main_page.enter_search_text(Keys.ENTER)
    yandex_search_result_page = SearchResultPage(browser, browser.current_url)
    assert yandex_search_result_page.search_result_exists(), "Отсутствует таблица результатов поиска"

    # 6. Проверить, что первая ссылка ведет на сайт tensor.ru
    first_link = yandex_search_result_page.get_first_link()
    assert first_link == "https://tensor.ru/" or first_link == "http://tensor.ru/", \
...
def test_images_search(browser):
    ...
    assert main_page.is_images_link_present(), "Ссылка 'Картинки' отсутствует на странице"
    # 3. Кликаем на ссылку
    main_page.click_images_link()
    browser.switch_to.window(browser.window_handles[-1])

    # 4. Проверить, что перешли на url https://yandex.ru/images/
    assert browser.current_url.startswith("https://yandex.ru/images/"), "Переход на неверный url"
    images_page = ImagesPage(browser, browser.current_url)
    # 5. Открыть первую категорию
    first_category_name = images_page.open_first_category_and_get_name()
    images_search_result_page = ImagesSearchResultPage(browser, browser.current_url)

    # 6. Проверить, что название категории отображается в поле поиска
    assert images_search_result_page.get_text_in_search_field() == first_category_name, \
    ...
    assert images_viewer_page.get_current_image_src() != first_image_src, "Картинка не сменилась"
    # 11. Нажать назад
